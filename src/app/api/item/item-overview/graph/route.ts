...
export async function GET(req: Request) {
  ...
  const data = await sql(
    `SELECT 'revenue' AS type, ${date}, SUM((COALESCE(price, 0) - COALESCE(discount, 0)) * COALESCE(quantity, 1)) AS revenue FROM orders JOIN order_items ON order_items.order_id = orders.id WHERE orders.store_id = $1 AND orders.status = 'completed' AND ${timeConstraint(
      ...
      undefined,
      "completed_on",
    )} GROUP BY date UNION SELECT 'potential' AS type, ${date}, SUM((COALESCE(price, 0) - COALESCE(discount, 0)) * COALESCE(quantity, 1)) AS revenue FROM orders JOIN order_items ON order_items.order_id = orders.id WHERE orders.store_id = $1 AND order_items.item_id = $2 AND (orders.status = 'cancelled' OR orders.status = 'customer_cancelled') AND ${timeConstraint(
      range,
      undefined,
      "completed_on",
    )} GROUP BY date`,
