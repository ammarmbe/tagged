...
export async function POST(req: Request) {
  ...
  let query =
    "SELECT items.id AS item_id, nano_id, array_unique(array_agg(color_value)) as colors, (CASE WHEN (SELECT SUM(quantity) FROM item_details WHERE item_id = items.id) > 0 THEN false ELSE true END) as out_of_stock, items.name AS item_name, items.description AS description, price, discount, users.name AS store_name, users.id AS store_id FROM items JOIN item_details ON item_details.item_id = items.id JOIN users ON users.id = items.store_id WHERE items.id > $1 AND store_id = $2 AND items.deleted != true";
