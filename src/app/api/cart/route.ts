...
export async function GET() {
  ...
  const data = await sql(
    "SELECT items.name as item_name, items.nano_id, (SELECT url FROM item_images WHERE item_id = items.id ORDER BY thumbnail LIMIT 1) AS image_url, users.nano_id as store_nano_id, cart_items.id as id, users.name as store_name, users.id as store_id, items.id as item_id, items.price, COALESCE(items.discount, 0) as discount, cart_items.quantity as quantity, item_configs.size as size, item_configs.color as color FROM cart_items JOIN items ON items.id = cart_items.item_id JOIN users ON users.id = items.store_id JOIN item_configs ON item_configs.id = cart_items.item_config_id WHERE user_id = $1",
    [user.id],
  );
